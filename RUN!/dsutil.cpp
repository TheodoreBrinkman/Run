...
//-----------------------------------------------------------------------------
// Name: cSoundManagerDX::cSoundManagerDX()
// Desc: Constructs the class
//-----------------------------------------------------------------------------
cSoundManagerDX::cSoundManagerDX()
{
    m_pDS = NULL;
}
...
//-----------------------------------------------------------------------------
// Name: cSoundManagerDX::Initialize()
// Desc: Initializes the IDirectSound object and also sets the primary buffer
//       format.  This function must be called before any others.
//-----------------------------------------------------------------------------
HRESULT cSoundManagerDX::Initialize( HWND  hWnd, 
                                   DWORD dwCoopLevel )
{
    HRESULT             hr;
...
//-----------------------------------------------------------------------------
// Name: cSoundManagerDX::SetPrimaryBufferFormat()
...
//               dwPrimaryBitRate  = 16
//-----------------------------------------------------------------------------
HRESULT cSoundManagerDX::SetPrimaryBufferFormat( DWORD dwPrimaryChannels, 
                                               DWORD dwPrimaryFreq, 
...
//-----------------------------------------------------------------------------
// Name: cSoundManagerDX::Get3DListenerInterface()
// Desc: Returns the 3D listener interface associated with primary buffer.
//-----------------------------------------------------------------------------
HRESULT cSoundManagerDX::Get3DListenerInterface( LPDIRECTSOUND3DLISTENER* ppDSListener )
{
    HRESULT             hr;
...
//-----------------------------------------------------------------------------
// Name: cSoundManagerDX::Create()
// Desc: 
//-----------------------------------------------------------------------------
HRESULT cSoundManagerDX::Create( cSoundDX** ppSound, 
                               ...
                               DWORD dwNumBuffers )
{
    ...
    // Create the sound
    *ppSound = new cSoundDX( apDSBuffer, dwDSBufferSize, dwNumBuffers, pWaveFile, dwCreationFlags );
...
//-----------------------------------------------------------------------------
// Name: cSoundManagerDX::CreateFromMemory()
// Desc: 
//-----------------------------------------------------------------------------
HRESULT cSoundManagerDX::CreateFromMemory( cSoundDX** ppSound, 
                                        BYTE* pbData,
                                        ULONG  ulDataSize,
                                        LPWAVEFORMATEX pwfx,
                                        DWORD dwCreationFlags, 
...
//-----------------------------------------------------------------------------
// Name: cSoundManagerDX::CreateStreaming()
// Desc: 
//-----------------------------------------------------------------------------
HRESULT cSoundManagerDX::CreateStreaming( cStreamingSoundDX** ppStreamingSound, 
                                        LPTSTR strWaveFileName, 
                                        DWORD dwCreationFlags, 
                                        GUID guid3DAlgorithm,
                                        DWORD dwNotifyCount, 
                                        ...
                                        HANDLE hNotifyEvent )
{
    ...
    // Create the sound
    *ppStreamingSound = new cStreamingSoundDX( pDSBuffer, dwDSBufferSize, pWaveFile, dwNotifySize );
...
//-----------------------------------------------------------------------------
// Name: cSoundDX::cSoundDX()
// Desc: Constructs the class
//-----------------------------------------------------------------------------
cSoundDX::cSoundDX( LPDIRECTSOUNDBUFFER* apDSBuffer, DWORD dwDSBufferSize, 
                DWORD dwNumBuffers, cWaveFileDX* pWaveFile, DWORD dwCreationFlags )
{
    DWORD i;
...
//-----------------------------------------------------------------------------
// Name: cSoundDX::FillBufferWithSound()
// Desc: Fills a DirectSound buffer with a sound file 
//-----------------------------------------------------------------------------
HRESULT cSoundDX::FillBufferWithSound( LPDIRECTSOUNDBUFFER pDSB, BOOL bRepeatWavIfBufferLarger )
{
    HRESULT hr; 
...
//-----------------------------------------------------------------------------
// Name: cSoundDX::RestoreBuffer()
// Desc: Restores the lost buffer. *pbWasRestored returns TRUE if the buffer was 
//       restored.  It can also NULL if the information is not needed.
//-----------------------------------------------------------------------------
HRESULT cSoundDX::RestoreBuffer( LPDIRECTSOUNDBUFFER pDSB, BOOL* pbWasRestored )
{
    HRESULT hr;
...
//-----------------------------------------------------------------------------
LPDIRECTSOUNDBUFFER cSoundDX::GetFreeBuffer()
{
	DWORD i=0;
...
//-----------------------------------------------------------------------------
// Name: cSoundDX::GetBuffer()
// Desc: 
//-----------------------------------------------------------------------------
LPDIRECTSOUNDBUFFER cSoundDX::GetBuffer( DWORD dwIndex )
{
    if( m_apDSBuffer == NULL )
        return NULL;
...
//-----------------------------------------------------------------------------
// Name: cSoundDX::Get3DBufferInterface()
// Desc: 
//-----------------------------------------------------------------------------
HRESULT cSoundDX::Get3DBufferInterface( DWORD dwIndex, LPDIRECTSOUND3DBUFFER* ppDS3DBuffer )
{
    if( m_apDSBuffer == NULL )
        return CO_E_NOTINITIALIZED;
    if( dwIndex >= m_dwNumBuffers )
...
//-----------------------------------------------------------------------------
HRESULT cSoundDX::Play( DWORD dwPriority, DWORD dwFlags, LONG lVolume, LONG lFrequency, LONG lPan )
{
    HRESULT hr;
    BOOL    bRestored;
...
//-----------------------------------------------------------------------------
// Name: cSoundDX::Play3D()
// Desc: Plays the sound using voice management flags.  Pass in DSBPLAY_LOOPING
//       in the dwFlags to loop the sound
//-----------------------------------------------------------------------------
HRESULT cSoundDX::Play3D( LPDS3DBUFFER p3DBuffer, DWORD dwPriority, DWORD dwFlags, LONG lFrequency )
{
    HRESULT hr;
    BOOL    bRestored;
...
//-----------------------------------------------------------------------------
// Name: cStreamingSoundDX::cStreamingSoundDX()
...
//-----------------------------------------------------------------------------
cStreamingSoundDX::cStreamingSoundDX( LPDIRECTSOUNDBUFFER pDSBuffer, DWORD dwDSBufferSize, 
                                  cWaveFileDX* pWaveFile, DWORD dwNotifySize ) 
                : cSoundDX( &pDSBuffer, dwDSBufferSize, 1, pWaveFile, 0 )           
{
    m_dwLastPlayPos     = 0;
    m_dwPlayProgress    = 0;
...
// Desc: Destroys the class
//-----------------------------------------------------------------------------
cStreamingSoundDX::~cStreamingSoundDX()
{
}
...
//-----------------------------------------------------------------------------
// Name: cStreamingSoundDX::HandleWaveStreamNotification()
// Desc: Handle the notification that tells us to put more wav data in the 
//       circular buffer
//-----------------------------------------------------------------------------
HRESULT cStreamingSoundDX::HandleWaveStreamNotification( BOOL bLoopedPlay )
{
    HRESULT hr;
...
//-----------------------------------------------------------------------------
// Name: cWaveFileDX::cWaveFileDX()
// Desc: Constructs the class.  Call Open() to open a wave file for reading.  
//       Then call Read() as needed.  Calling the destructor or Close() 
//       will close the file.  
//-----------------------------------------------------------------------------
cWaveFileDX::cWaveFileDX()
{
    m_pwfx    = NULL;
    m_hmmio   = NULL;
...
//-----------------------------------------------------------------------------
// Name: cWaveFileDX::Open()
// Desc: Opens a wave file for reading
//-----------------------------------------------------------------------------
HRESULT cWaveFileDX::Open( LPTSTR strFileName, WAVEFORMATEX* pwfx, DWORD dwFlags )
{
    HRESULT hr;
...
//-----------------------------------------------------------------------------
// Name: cWaveFileDX::OpenFromMemory()
// Desc: copy data to cWaveFileDX member variable from memory
//-----------------------------------------------------------------------------
HRESULT cWaveFileDX::OpenFromMemory( BYTE* pbData, ULONG ulDataSize, 
...
//-----------------------------------------------------------------------------
// Name: cWaveFileDX::ReadMMIO()
// Desc: Support function for reading from a multimedia I/O stream.
//       m_hmmio must be valid before calling.  This function uses it to
//       update m_ckRiff, and m_pwfx. 
//-----------------------------------------------------------------------------
HRESULT cWaveFileDX::ReadMMIO()
{
...
//-----------------------------------------------------------------------------
// Name: cWaveFileDX::GetSize()
// Desc: Retuns the size of the read access wave file 
//-----------------------------------------------------------------------------
DWORD cWaveFileDX::GetSize()
{
    return m_dwSize;
}
...
//-----------------------------------------------------------------------------
// Name: cWaveFileDX::ResetFile()
// Desc: Resets the internal m_ck pointer so reading starts from the 
//       beginning of the file again 
//-----------------------------------------------------------------------------
HRESULT cWaveFileDX::ResetFile()
{
    if( m_bIsReadingFromMemory )
    {
        m_pbDataCur = m_pbData;
...
//       Reset() is called.
//-----------------------------------------------------------------------------
HRESULT cWaveFileDX::Read( BYTE* pBuffer, DWORD dwSizeToRead, DWORD* pdwSizeRead )
{
    if( m_bIsReadingFromMemory )
    {
        if( m_pbDataCur == NULL )
...
//-----------------------------------------------------------------------------
// Name: cWaveFileDX::Close()
// Desc: Closes the wave file 
//-----------------------------------------------------------------------------
HRESULT cWaveFileDX::Close()
{
    if( m_dwFlags == WAVEFILE_READ )
    {
        mmioClose( m_hmmio, 0 );
...
//-----------------------------------------------------------------------------
// Name: cWaveFileDX::WriteMMIO()
// Desc: Support function for reading from a multimedia I/O stream
//       pwfxDest is the WAVEFORMATEX for this new wave file.  
//       m_hmmio must be valid before calling.  This function uses it to
//       update m_ckRiff, and m_ck.  
//-----------------------------------------------------------------------------
HRESULT cWaveFileDX::WriteMMIO( WAVEFORMATEX *pwfxDest )
{
...
//-----------------------------------------------------------------------------
// Name: cWaveFileDX::Write()
// Desc: Writes data to the open wave file
//-----------------------------------------------------------------------------
HRESULT cWaveFileDX::Write( UINT nSizeToWrite, BYTE* pbSrcData, UINT* pnSizeWrote )
{
    UINT cT;
